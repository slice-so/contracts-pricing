...
interface ISlicer is IERC721Receiver, IERC1155Receiver {
    ...
    function batchReleaseAccounts(address[] memory accounts, address currency, bool withdraw) external;

    function unreleased(address account, address currency) external view returns (uint256 unreleasedAmount);
    ...
    function _updatePayeesReslice(address payable[] memory accounts, int32[] memory tokensDiffs, uint32 totalSupply)
        external;
    ...
    function _handle1155Purchase(address buyer, address contractAddress, uint256 quantity, uint256 tokenId) external;
}
