...
contract ERC721Discount is ISliceProductPrice {
     ...
     */
    function _getDiscount(DiscountParams memory params, address buyer) internal view returns (uint256 discount) {
        ...
        for (uint256 i; i < discountsRef.length;) {
            ...
            if (IERC721(discountsRef[i].nftAddress).balanceOf(buyer) >= discountsRef[i].minQuantity) {
